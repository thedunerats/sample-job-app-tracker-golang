...
func TestCreateJob(t *testing.T) {
	...
	now := time.Now()
	result, err := db.Exec(query, "Google", "Software Engineer", "applied", now,
...
func TestGetJob(t *testing.T) {
	...
	insertQuery := `INSERT INTO job_applications (company, position, status, applied_date, 
				   notes, contact_info, salary, location, created_at, updated_at) 
				   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	
	now := time.Now()
	result, _ := db.Exec(insertQuery, "Microsoft", "Cloud Engineer", "interview", now,
...
func TestUpdateJob(t *testing.T) {
	...
	now := time.Now()
	result, _ := db.Exec(insertQuery, "Amazon", "DevOps Engineer", "applied", now,
...
func TestDeleteJob(t *testing.T) {
	...
	now := time.Now()
	result, _ := db.Exec(insertQuery, "Apple", "iOS Developer", "rejected", now,
...
func TestGetJobsByStatus(t *testing.T) {
	...
	now := time.Now()
	db.Exec(insertQuery, "Company A", "Engineer A", "applied", now, "", "", "", "", now, now)
	db.Exec(insertQuery, "Company B", "Engineer B", "interview", now, "", "", "", "", now, now)
	db.Exec(insertQuery, "Company C", "Engineer C", "applied", now, "", "", "", "", now, now)
	db.Exec(insertQuery, "Company D", "Engineer D", "offer", now, "", "", "", "", now, now)
